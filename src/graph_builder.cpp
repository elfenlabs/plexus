...
void GraphBuilder::add_node(NodeConfig config) {
...
ExecutionGraph GraphBuilder::bake() {
    ...
    // Actually, m_nodes is already in insertion order. 
    // If we want to support priority, we should stable_sort.
    // For now, let's assume registration order is enough or simple priority sort.
     std::stable_sort(m_nodes.begin(), m_nodes.end(), [](const NodeConfig& a, const NodeConfig& b) {
    ...
    // Resources tracking
    // We don't know the max ResourceID ahead of time easily without querying Context or dynamic sizing.
    // We'll use a map for sparse tracking or resize if we knew max ID. 
    // Given Context::m_next_id, we could ask Context, but for now map is safer.
        ...
        for (const auto& dep : node.dependencies) {
            ...
            if (dep.access == Access::READ) {
                ...
                if (last_writer.count(res)) {
                    ...
                    // Checking existence in vector is O(E), let's just add and assume ok or use set if needed.
